...
@pytest.mark.parametrize(
    "nome_model",
    ["categoria", "coletivo", "pessoa", "evento", "pontodeinteresse", "usuario"],
)
def test_formulario_de_edicao_abre(admin_logado, registros, nome_model):
...
def test_contagem_de_pessoas_nao_infla_com_filtro_de_categoria(
    ...
    A contagem é feita por `annotate(..., distinct=True)`; sem o `distinct`, um
    coletivo em duas categorias apareceria com o dobro de pessoas.
    """
    coletivo = registros["coletivo"]
    coletivo.categorias.add(Categoria.objects.create(nome="Alimentação", slug="alim"))

    url = reverse("admin:rede_coletivo_changelist")
    resposta = admin_logado.get(
        url, {"categorias__id__exact": registros["categoria"].pk}
    )
    linha = resposta.context["cl"].result_list.get(pk=coletivo.pk)

    assert linha._num_pessoas == coletivo.pessoas.count() == 1
